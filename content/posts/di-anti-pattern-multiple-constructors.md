...
From a DI perspective, your applications have two kinds of types: [newables and injectables](https://web.archive.org/web/20210429205925/http://misko.hevery.com/2008/09/30/to-new-or-not-to-new/). Newables are classes that the application news up manually using the `new` keyword. This is true for types such as primitives, entities, [DTOs](http://en.wikipedia.org/wiki/Data_transfer_object), view models and messages. Newables contain little to no logic and application code can safely depend on their implementation; there is no need to hide them behind an abstraction.
